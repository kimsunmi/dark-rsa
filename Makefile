...
clean:
	rm -f *.a *.o $(TEST_SETUP) $(TEST_COMMIT) $(TEST_PROVER_EVAL) $(TEST_VERIFIER_EVAL)
...
$(TEST_PROVER_EVAL): $(TEST_SOURCE_DIR)eval_prover_test.c 
	$(CC) -o  $@  $(TEST_SOURCE_DIR)eval_prover_test.c $(SOURCE_DIR)setup.c $(SOURCE_DIR)commit.c $(SOURCE_DIR)evaluate.c $(SOURCE_DIR)util.c $(SOURCE_DIR)poe.c $(SOURCE_DIR)codeTimer.c $(CFLAGS) -g -fsanitize=address

$(TEST_VERIFIER_EVAL): $(TEST_SOURCE_DIR)eval_verify_test.c 
	$(CC) -o $@ $(TEST_SOURCE_DIR)eval_verify_test.c $(SOURCE_DIR)setup.c $(SOURCE_DIR)commit.c $(SOURCE_DIR)evaluate.c $(SOURCE_DIR)util.c $(SOURCE_DIR)poe.c $(SOURCE_DIR)codeTimer.c $(CFLAGS)  -g -fsanitize=address
